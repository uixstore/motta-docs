...
1. **Use a content delivery network (CDN).**
	...
	Essentially, a CDN, or content delivery network, takes all your static files you’ve got on your site (CSS, Javascript and _images etc) and lets visitors download them as fast as possible by serving the files on servers as close to them as possible.

1. **Optimize _images (automatically)**

	Yahoo! has an image optimizer called Smush.it that will drastically reduce the file size of an image, while not reducing quality. However, if you are like me, doing this to every image would be beyond a pain, and incredibly time consuming. Fortunately, there is an amazing, free plugin called [WP-SmushIt](https://wordpress.org/extend/plugins/wp-smushit/) which will do this process to all of your _images automatically, as you are uploading them. No reason not to install this one.
